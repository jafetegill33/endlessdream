...
export function createControls(camera, domElement) {
  ...
  const lookSensitivity = isTouchDevice ? 0.0032 : 0.0022;
  ...
  if (!isTouchDevice) {
    ...
    lookArea.addEventListener('touchstart', (e) => {
      // ignore touches that start on the joystick so look + move don't conflict
      if (joystickZone && e.target && e.target.closest('#joystick-zone')) return;
      e.preventDefault();
    ...
    lookArea.addEventListener('touchend', endDrag, { passive: false });
    lookArea.addEventListener('touchcancel', endDrag, { passive: false });
  }
